<script lang="ts">
  ...
  type ManifestLoader = () => Promise<IconManifest>;
  type ManifestUrlResolver = (
    manifest: IconManifest,
    repoConfig: RepoConfig | null
  ) => string;

  export let loadManifest: ManifestLoader;
  export let sourceKey: string;
  export let breadcrumbHref = '/';
  export let breadcrumbLabel = '图标集合';
  export let resolveManifestUrl: ManifestUrlResolver =
    defaultManifestUrlResolver;
  export let shouldRefreshRepoConfig = true;
  export let shouldRefreshOnAuthChange = true;
  ...
  $: manifestUrl = manifest ? resolveManifestUrl(manifest, repoConfig) : '';
  ...
  $: if (mounted && sourceKey !== activeSourceKey) {
    activeSourceKey = sourceKey;
    void refreshView();
  }

  /// 生成默认的 manifest 复制地址，保持现有集合页行为。
  function defaultManifestUrlResolver(
    currentManifest: IconManifest,
    currentRepoConfig: RepoConfig | null
  ) {
    return manifestRawUrl(currentManifest.id, currentRepoConfig);
  }

  /// 刷新当前登录用户的仓库配置，用于复制 GitHub Raw manifest 地址。
  async function refreshRepoConfig() {
    if (!shouldRefreshRepoConfig) {
      repoConfig = null;
      return;
    }
  ...
  function copyShareUrl() {
    ...
    void copyUrl(sharePageUrl(manifestUrl));
  }
  ...
  onMount(() => {
    ...
    const handleAuthChanged = () => {
      if (!shouldRefreshOnAuthChange) return;
      void refreshView();
    };
